...
export function FAQSection() {
  ...
  return (
    <section id="faq" className="max-w-6xl mx-auto px-5 sm:px-8 md:px-12 py-20 md:py-28 border-t border-[#1a1a1a]">
      <RevealOnScroll>
      ...
      <RevealOnScroll delay={80} className="mt-12 md:mt-14 max-w-3xl">
        {FAQS.map((faq, i) => {
          const isOpen = open === i;
          return (
            <div key={faq.q} className="border-b border-[#1a1a1a] cursor-pointer group" onClick={() => setOpen(isOpen ? null : i)}>
              <div className="flex items-center justify-between py-5 sm:py-6 gap-4">
                <span className="text-sm sm:text-base font-medium text-white group-hover:text-[#c96acb] transition-colors duration-200" style={{ letterSpacing: "-0.01em" }}>
                  {faq.q}
                </span>

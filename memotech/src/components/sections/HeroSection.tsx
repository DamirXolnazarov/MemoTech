...
function PersonSVG({ wrapRef }: { wrapRef: React.RefObject<HTMLDivElement | null> }) {
  return (
    <div
      ref={wrapRef}
      style={{
        position: "absolute", left: "5vw", top: "50%", transform: "translateY(-52%)",
        zIndex: 6, pointerEvents: "none", opacity: 0,
      }}
    >
      <svg width="280" height="280" viewBox="0 0 64 64" fill="white" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet">
...
function MouthWaves({ waveRefs }: { waveRefs: React.MutableRefObject<SVGPathElement[]> }) {
  const arcs = [
  ...
  return (
    ...
    >
      <g transform="translate(340,450)">
        {arcs.map((d, i) => (
          <path key={i} ref={(el) => { if (el) waveRefs.current[i] = el; }} d={d} fill="none" stroke={ORCHID} strokeWidth={2.6 - i * 0.35} strokeLinecap="round" opacity={0} />
        ))}
      </g>
    </svg>
  );
...
function RadialCircles({ circleRefs }: { circleRefs: React.MutableRefObject<SVGCircleElement[]> }) {
  return (
    <svg
    ...
    >
      <g transform="translate(720,440)">
        {[55, 120, 195, 280, 375, 480].map((r, i) => (
          <circle key={i} ref={(el) => { if (el) circleRefs.current[i] = el; }} cx={0} cy={0} r={r} fill="none" stroke={ORCHID} strokeWidth={1.8 - i * 0.18} opacity={0} />
        ))}
      </g>
    </svg>
  );
...
const CARD_POSITIONS = [
  ...
  { left: "67%", top: "42%", transform: "translateX(0)" },
  { left: "50%", top: "67%", transform: "translateX(-50%)" },
  { left: "20%", top: "42%", transform: "translateX(0)" },
      ...
      {CARDS.map((card, i) => (
        <div
          ...
          style={{
            ...
            transform: CARD_POSITIONS[i].transform,
            width: 185,
      ...
      {PHASES.map((label, i) => (
        <div key={label} style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div
            ...
            style={{ width: 16, height: 16, borderRadius: "50%", border: `1.5px solid rgba(201,106,203,0.35)`, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}
          >
            <div
              ref={(el) => { if (el) activeDotRefs.current[i] = el; }}
              style={{ width: 7, height: 7, borderRadius: "50%", background: ORCHID, boxShadow: `0 0 8px ${ORCHID}`, opacity: i === 0 ? 1 : 0, transition: "opacity 0.3s ease" }}
            />
          </div>
...
/* ─── Desktop animated hero (unchanged GSAP scroll experience) ──────────── */
function DesktopHero() {
  ...
  useEffect(() => {
    ...
    const gsapCtx = gsap.context(() => {
      ...
      tl.fromTo(personWrapRef.current, { opacity: 0, x: -120 }, { opacity: 1, x: 0, duration: 0.14, ease: "power3.out" }, 0.18);
      mouthWaveRefs.current.forEach((w, i) => {
        const s = 0.22 + i * 0.026;
        tl.fromTo(w, { opacity: 0, scale: 0.3, transformOrigin: "0px 0px" }, { opacity: 0.8 - i * 0.12, scale: 1, transformOrigin: "0px 0px", duration: 0.10, ease: "power2.out" }, s);
      ...
      tl.to(micWrapRef.current, { scale: 2.8, duration: 0.16, ease: "power3.inOut", transformOrigin: "50% 50%" }, 0.38);

      circleRefs.current.forEach((c, i) => {
        tl.fromTo(c, { attr: { r: 0 }, opacity: 0 }, { attr: { r: 55 + i * 85 }, opacity: 0.55 - i * 0.07, duration: 0.14, ease: "power2.out" }, 0.42 + i * 0.022);
      });

      tl.to(micWrapRef.current, { scale: 8, opacity: 0, duration: 0.16, ease: "power3.in", transformOrigin: "50% 50%" }, 0.60);

      tl.to(".radial-circles-svg", { scale: 3.6, transformOrigin: "50% 49%", duration: 0.20, ease: "power3.in" }, 0.60);

      tl.to(cardsWrapRef.current, { opacity: 1, duration: 0.04 }, 0.68);
      cardRefs.current.forEach((card, i) => {
        tl.fromTo(card, { opacity: 0, scale: 0.86, y: 18 }, { opacity: 1, scale: 1, y: 0, duration: 0.08, ease: "back.out(1.5)" }, 0.70 + i * 0.030);
      });

      tl.to([cardsWrapRef.current, ".radial-circles-svg"], { opacity: 0, duration: 0.07 }, 0.83);
      tl.fromTo(headlineRef.current, { opacity: 0, y: 28 }, { opacity: 1, y: 0, duration: 0.07, ease: "power2.out" }, 0.90);
    }, wrapperRef);
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            alignItems: "center", justifyContent: "center", gap: 22, opacity: 0,
            zIndex: 15, textAlign: "center", padding: "0 24px", pointerEvents: "none",
          }}
        >
          <h1
            style={{
              fontFamily: "var(--font-syne, Syne, sans-serif)", fontWeight: 800,
              fontSize: "clamp(2.8rem, 5.5vw, 5rem)", lineHeight: 1.07, letterSpacing: "-0.02em", color: "white",
            }}
          >
          ...
          <p
            style={{
              fontFamily: "var(--font-inter, Inter, sans-serif)", fontSize: "clamp(0.95rem, 1.4vw, 1.1rem)",
              color: "#a1a1aa", maxWidth: 420, lineHeight: 1.65,
            }}
          >
        ...
        <div
          ...
          style={{
            ...
            display: "flex", flexDirection: "column", alignItems: "center", gap: 8, zIndex: 20, pointerEvents: "none",
          }}
        >
...
/* ─── Mobile static hero — simple, fast, no scroll-jacking, no overflow ──── */
function MobileHero() {
  return (
    <div
      style={{
        ...
        background: `
          ...
          radial-gradient(ellipse 70% 40% at 50% 85%, rgba(201,106,203,0.10) 0%, transparent 60%),
          #050505
        `,
      }}
    >
      <div
        ...
        style={{
          ...
          marginBottom: 28,
          boxShadow: "0 0 40px rgba(201,106,203,0.2)",
        }}
      >
        <Mic size={32} color={ORCHID} strokeWidth={2} />
      </div>
      ...
      >
        Memo listens, understands, and surfaces exactly what you need — the moment you need it.
      </p>
      ...
      {/* Mini preview cards — static, no animation, gives a taste of the product */}
      <div className="flex flex-col gap-2.5 w-full mt-10" style={{ maxWidth: 320 }}>
        {CARDS.slice(0, 3).map((card) => (
          <div
            key={card.label}
            style={{
              background: "rgba(8,8,8,0.6)",
              border: "1px solid rgba(201,106,203,0.18)",
              borderRadius: 14,
              padding: "12px 16px",
              textAlign: "left",
            }}
          >
            <div style={{ display: "flex", alignItems: "center", gap: 7, fontFamily: "var(--font-syne, Syne, sans-serif)", fontWeight: 700, fontSize: 11, color: ORCHID, letterSpacing: "0.06em", marginBottom: 6 }}>
              {card.icon}
              {card.label}
            </div>
            <p style={{ fontSize: 12, lineHeight: 1.6, color: "#a1a1aa", margin: 0 }}>
              {card.body}
            </p>
          </div>
        ))}
...
/* ─── Main export — desktop gets the animated scroll experience,
     mobile gets a fast static hero ──────────────────────────────────────── */
